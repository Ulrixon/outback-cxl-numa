...
#include "hashutils/hash.h"
#include "tables/cuckoo_ht.h"
#include "tables/othello_cp_dp.h"
...
public:
  ...
  explicit DataPlaneLudo(const ControlPlaneLudo<K, V, VL> &cp,
                        LudoBuckets*& pd) : locator(cp.locator) {
    h = cp.h;
    digestH = cp.digestH;
    num_buckets_ = cp.num_buckets_;
    resetMemory();

    pd = new LudoBuckets(cp.num_buckets_);
    ...
    for (uint32_t bktIdx = 0; bktIdx < num_buckets_; ++bktIdx) {
      ...
      uint64_t length = 64UL << 48;

      LudoBucket &pdBucket = pd->bucketsArray[bktIdx];
      memset(pdBucket.slots, 0, kSlotsPerBucket * sizeof(uint64_t));

      const FastHasher64<K> locateHash(cpBucket.seed);
      for (char slot = 0; slot < kSlotsPerBucket; ++slot) {
        if (cpBucket.occupiedMask & (1U << slot)) {
          const K &k = cpBucket.keys[slot];
          pdBucket.slots[locateHash(k) >> 62] = cpBucket.values[slot] + length; //
        }
      }
    }
  }
  ...
  // Returns true if found.  Sets *out = value. No use it
  inline bool lookUp(const K &k, V &out) const {
    uint32_t bktId[2];
    ...
    while (true) {
      ...
      uint32_t bktIdt = bktId[locator.lookUp(k)];
      const Bucket &bucket = buckets[bktIdt];
  ...
  inline void updateSeed(const uint64_t bktIdx, const uint8_t seed) {
    buckets[bktIdx].seed = seed;
  }

  // Returns true if found.  Sets *out = value.
  inline pair<uint32_t, uint8_t> lookup_slot(const K &k) const {
    uint32_t bktId[2];
    fast_map_to_buckets(h(k), bktId, num_buckets_);
  ...
  inline uint64_t getBucketsNum() const {
    return buckets.size();
  }
