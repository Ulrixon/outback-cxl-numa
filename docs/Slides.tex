...
\begin{frame}{RDMA: YCSB Throughput (Paper Figs.~9--10, Ours: 1\,MN+2\,CN)}
\begin{columns}[T]
% ── Left: chart ──────────────────────────────────────────────────────────────
...
\begin{block}{RDMA Reproduction (r320 / CX-3, 8\,CN $\times$ 8\,T)}
\begin{itemize}\setlength{\itemsep}{1pt}
  \item \textbf{Throughput:} 5.04\,MOPS peak (CX-3, 1\,MN+2\,CN, 32T) vs.\ paper's 6.01\,MOPS (CX-6, 9-node) — \textbf{84\%} of claimed; saturates at $\sim$5\,MOPS (MN-thread bound)
  \item \textbf{Memory:} DMPH index \textbf{16.87\,MB} (vs.\ claimed 12.5\,MB, $+35\%$); process RSS \textbf{915.8\,MB} — never reported in paper
  \item \textbf{Writes:} YCSB-F throughput degrades at 32\,T; paper omits this regime
  \item \textbf{Resize:} \textbf{45\%} throughput drop (vs.\ paper's $\sim$52\%) — consistent trend but separate undocumented branch required
  \item \textbf{Codebase gaps:} broken Zipfian generator; no YCSB-E implementation
\end{itemize}
\end{block}
