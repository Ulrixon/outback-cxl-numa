...
\begin{frame}{Three Platforms Compared}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
 & \textbf{Paper (r650)} & \textbf{RDMA Repro (ours)} & \textbf{CXL/NUMA (c6420)} \\
\midrule
NIC & CX-6, 100\,Gbps & CX-3, 56\,Gbps FDR & N/A (load/store) \\
CPU & Gold 6338N, 32C & E5-2450, 8C/16T & 2$\times$Gold 6142, 16C \\
Memory & 256\,GB & 64\,GB & 384\,GB \\
Nodes & Up to 9 & 3 & 1 \\
Max Threads & 144/shard & 32 (2 CN) & 32 (1 socket) \\
Interconnect & PCIe/RDMA & 56\,Gbps FDR IB & UPI $\sim$150\,ns \\
\bottomrule
\end{tabular}

\vspace{8pt}
\textbf{Expected:}\\
RDMA reproduction $\approx$ paper per-MN-thread rate\\
CXL/NUMA \textbf{$\gg$} both (no network stack)
\end{frame}
...
\begin{enumerate}
  ...
  \medskip
  \item \textbf{Memory requirements undisclosed} --- 80M keys causes OOM during server bring-up on our 64\,GB nodes (huge-page MR + raw KV + DMPH index exhaust the huge-page pool)\\
...
\begin{axis}[
    ...
    xmin=1, xmax=200,
    ymin=0, ymax=20,
    legend style={at={(0.5,-0.52)},anchor=north,legend columns=5,font=\tiny},
    every axis plot/.append style={mark size=1.5pt, thick},
    grid=major, grid style={gray!30},
    title style={font=\small},
    title={Ours vs.\ Fig.~9 (CX-6) \& Fig.~10 (CX-3 4MN)}
]
% Our data (solid markers)
...
% Paper Fig.9 (CX-6, 1MN) -- densely dashed
...
\legend{C\,(ours), B\,(ours), A\,(ours), D\,(ours), F\,(ours),
        C\,(F9), B\,(F9), A\,(F9), D\,(F9), F\,(F9),
        C\,(F10), B\,(F10), A\,(F10), D\,(F10), F\,(F10)}
\end{axis}
\end{tikzpicture}
...
65--84\% of CX-6 peaks despite 2$\times$ weaker NIC\\
        (56\,Gbps FDR vs.\ 100\,Gbps CX-6).\\
Per-MN-thread rate is consistent.
\end{block}
\end{columns}
\end{frame}
...
\begin{frame}{CXL/NUMA: Near-Perfect Linear Scaling}
\begin{columns}[T]
...
\begin{itemize}
  \item \textbf{118 MOPS} at 32 threads
  \item Mean latency: \textbf{0.14\,$\mu$s}\\
        (vs.\ 3.04\,$\mu$s RDMA)
  \bigskip
  \item Near-perfect linear scaling:\\
        each thread adds $\sim$3.7\,MOPS
  \bigskip
  \item No NIC bottleneck:\\
        throughput $\propto$ thread count
  \bigskip
  \item P99 latency: 0.50\,$\mu$s\\
        P999 latency: 0.70\,$\mu$s
\end{itemize}
\end{columns}
\end{frame}
...
% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Resize Behavior: RDMA (Paper Fig.~17)}
\centering
\begin{tikzpicture}
\begin{axis}[
    width=0.85\textwidth,
    ...
    grid=major, grid style={gray!20},
    title style={font=\small},
    title={RDMA Resize Time Series (20M keys, YCSB-D)},
...
\addlegendentry{16T}
% Paper Fig.~17 curves (CX-6, r650, digitized -- first 24 s)
\addplot[color=papercolor!60,mark=none,thin] coordinates {
...
\small Bold = our CX-3 (4/8/12/16T); thin green = Paper Fig.~17 (CX-6, first 24\,s). Paper: smooth $\sim$52\% dip, $\sim$1.5\,s; ours: erratic post-resize spikes overshooting pre-resize by up to 38\%.
\end{frame}

% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Resize Behavior: CXL/NUMA}
\centering
\begin{tikzpicture}
\begin{axis}[
    width=0.85\textwidth,
    ...
    grid=major, grid style={gray!20},
    title style={font=\small},
    title={CXL/NUMA Resize Time Series (20M keys, YCSB-D)},
...
\small $\sim$95\% stall for 1--12\,s; 16T R4 stall lasts $\sim$12\,s at high table occupancy
\end{frame}

% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Memory Usage: Paper vs.\ Reality --- LF\,=\,0.80 \& 0.85}
\begin{columns}[T]
\column{0.50\textwidth}
\centering
\begin{tikzpicture}
\begin{axis}[
    ...
    ylabel={DMPH Index (MB)},
    xtick={20,40,60,80,100},
    ymin=0, ymax=75,
    grid=major, grid style={gray!20},
    legend style={at={(0.02,0.98)},anchor=north west,font=\tiny,row sep=-2pt},
    every axis plot/.append style={thick},
    title style={font=\small},
    title={LF = 0.80}
]
\addplot[color=papercolor,dashed,mark=o,mark size=1.8pt]
  coordinates {(20,11.52)(40,23.03)(60,34.55)(80,46.06)(100,57.58)};
\addlegendentry{Paper Formula}
\addplot[color=papercolor!50!black,densely dashed,mark=pentagon*,mark size=1.8pt]
  coordinates {(20,12.5)(40,25)(60,37)(80,50)(100,58)};
\addlegendentry{Paper Fig.\,16 (est.)}
\addplot[color=rdmacolor,mark=square*,mark size=1.8pt]
  coordinates {(20,15.48)(40,30.95)(60,61.88)(80,61.88)(100,61.88)};
\addlegendentry{RDMA Measured}
\addplot[color=numacolor,mark=triangle*,mark size=1.8pt,densely dotted]
  coordinates {(20,15.47)(40,30.93)(60,61.87)(80,61.87)(100,61.87)};
\addlegendentry{CXL Measured}
\end{axis}
\end{tikzpicture}
...
\vspace{3pt}
\centering\small
Power-of-2 bucket allocation $\Rightarrow$ step-wise jumps; 60M jump = \textbf{72\% over} paper prediction
\end{frame}

% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{Memory Usage: Paper vs.\ Reality --- LF\,=\,0.90 \& 0.95}
\begin{columns}[T]
\column{0.50\textwidth}
\centering
\begin{tikzpicture}
...
\vspace{3pt}
\centering\small
LF\,=\,0.90: resize deferred to 80M (below threshold at 60M); LF\,=\,0.95: CXL slightly higher due to alignment
\end{frame}
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{RDMA (ours)} & \textbf{CXL/NUMA (c6420)} \\
\midrule
Peak YCSB-C        & 5.04\,MOPS & \textcolor{numacolor}{\textbf{118\,MOPS}} \\
Peak YCSB-D        & 4.28\,MOPS & \textcolor{numacolor}{\textbf{130\,MOPS}} \\
Mean Latency        & 3.04\,$\mu$s & \textcolor{numacolor}{\textbf{0.14\,$\mu$s}} \\
Scaling              & Sub-linear & \textcolor{numacolor}{\textbf{Near-linear}} \\
Bottleneck           & MN CPU + NIC & DRAM bandwidth \\
\bottomrule
\end{tabular}
...
\column{0.3\textwidth}
\centering
{\Huge\textcolor{numacolor}{\textbf{23$\times$}}}\\
\small throughput\\improvement
\column{0.3\textwidth}
\centering
{\Huge\textcolor{numacolor}{\textbf{21$\times$}}}\\
\small latency\\reduction
\column{0.3\textwidth}
\centering
{\Huge\textcolor{numacolor}{\textbf{0}}}\\
\small need for\\coroutines
\end{columns}
\end{frame}
