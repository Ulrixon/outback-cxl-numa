...
This study reproduced Outback's RDMA experiments and extended them to a CXL-emulated platform via NUMA inter-socket shared memory. The RDMA reproduction (Section~4) on CloudLab Apt \texttt{r320} nodes --- the same hardware class as the paper's Figure~10 cluster --- confirmed general per-MN-thread throughput trends but revealed significant discrepancies in resizing stability, memory overhead, write performance, and codebase completeness (Section~3). The CXL/NUMA extension (Section~5) demonstrated dramatic improvements:
