...
\midrule
CPU        & Xeon Gold 6338N, 32C/64T, 2.2\,GHz & Xeon E5-2450, 8C/16T, 2.1\,GHz \\
...
MN Threads & 1--3/shard, 2 shards                  & 4 \\
...
CPU & Xeon Gold 6338N, 32C & Xeon E5-2450, 8C & 2$\times$Xeon Gold 6142, 16C & RDMA: weaker per-shard CPU \\
Max Threads & 144 per shard & 32 (2$\times$16) & 32 (1 socket) & Cannot test high-thread behavior \\
MN Threads & 1--3 (2 shards) & 1--3 & N/A & RDMA limited by MN CPU \\
...
\begin{axis}[
    ...
    xmin=1, xmax=200,
    ymin=0, ymax=20,
    legend style={at={(0.5,-0.22)},anchor=north,font=\tiny,legend columns=5},
    every axis plot/.append style={mark size=2.5pt, thick},
    grid=major, grid style={gray!30},
    title={RDMA Repro vs.\ Paper Fig.~9 (CX-6, 1MN) \& Fig.~10 (CX-3, 4MN)}
]
% Our RDMA reproduction (1MN 1CN, coro=2) -- solid lines with markers
...
\legend{C\,(ours), B\,(ours), A\,(ours), D\,(ours), F\,(ours),
        C\,(F9 CX-6), B\,(F9), A\,(F9), D\,(F9), F\,(F9),
        C\,(F10 CX-3), B\,(F10), A\,(F10), D\,(F10), F\,(F10)}
\end{axis}
\end{tikzpicture}
\caption{Three-way RDMA comparison across setups. \textbf{Solid markers} = our r320 reproduction (1MN, 1CN, coro=2, up to 16T) using the same CX-3 MX354A IB FDR 56\,Gbps hardware as paper Fig.~10. \textbf{Dashed lines} = paper Fig.~9 Outback (CX-6 100\,Gbps, 1 MN thread, up to 144T). \textbf{Dotted lines} = paper Fig.~10 Outback (r320 / CX-3 MX354A IB FDR 56\,Gbps, 4 MN threads, up to 64T). Same color per workload across all three sources. Fig.~10 reaches up to 17\,MOPS because it aggregates 4 MN threads --- per-MN-thread throughput ($\sim$4.25\,MOPS) closely matches our 1-MN reproduction ($\sim$4.5\,MOPS), confirming consistent per-thread performance on the same r320 hardware. Fig.~9 (CX-6) saturates at 5.5--6\,MOPS per MN thread due to the faster NIC. YCSB-F (red) is the lowest across all setups due to the DMPH write path cost.}
