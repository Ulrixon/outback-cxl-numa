...
Beyond the paper's designed operating envelope, two critical memory limits were identified. (1)~\textbf{OOM at 80M keys (RDMA):} The r320's 16\,GB DRAM is insufficient for 80M KV pairs once the raw KV array, DMPH index, and pre-registered RDMA hugepage MR are all resident simultaneously --- the paper tests 80M only on r650 (256\,GB) without disclosing this constraint. (2)~\textbf{Swap thrashing at 60M keys:} At 60M keys with LF=0.80, throughput collapsed from $\sim$4.5\,MOPS to $\sim$2,945\,ops/s ($\sim$9.8\,ms latency) --- a \textbf{1,500$\times$} slowdown from memory swapping. Additional fragility points (write degradation at scale, coroutine overload, CXL resize stalls, range queries) are detailed in Sections~\ref{sec:codebase-issues}, \ref{sec:repro-coro}, \ref{sec:repro-mt}, and the three-way analysis (\S\ref{sec:repro-datasets}).
...
Detailed thread-by-thread three-way YCSB-C measurements are in Table~\ref{tab:three-way} (Appendix~\ref{app:tables}).
...
Both paper and RDMA scale near-linearly with MN threads (MN CPU is the bottleneck), while CXL/NUMA shows $<$2\% variation. Dataset identity (FB vs.\ OSM) has no measurable effect, confirming DMPH lookup cost is data-distribution-invariant under uniform access.

% ──────────────────────────────────────────────────────────────────────────────
\subsection{MN Thread Scalability (Three-Way)}
% ──────────────────────────────────────────────────────────────────────────────

The paper's Figure~12 varies MN threads 1--3 (4$\times$ r650, 288 CN threads): Outback is 1.10--1.21$\times$ Cluster and $\sim$3$\times$ MICA; peak at 2 MN threads is 9.89\,MOPS.

Our CXL/NUMA results show \textbf{no measurable impact} from mem\_threads ($<$2\% variance). Our RDMA confirms the paper's trend: 1T: 4.82, 2T: 7.92, 3T: 10.84\,MOPS (near-linear), because without RPC dispatch there is no MN CPU bottleneck on the CXL path.
...
In summary, all RDMA-based platforms (paper CX-6, paper CX-3, our r320) plateau at 4.3--6.0\,MOPS due to the 1 MN CPU thread bottleneck, while CXL/NUMA scales near-linearly to 118\,MOPS at 32 threads (limited only by DRAM bandwidth). See Table~\ref{tab:repro-summary} (\S\ref{sec:repro-ycsb}) and the discrepancy analysis in Appendix~\ref{app:tables} for full platform comparisons.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Resize incomplete:} Upstream resize code segfaults under RDMA load; our NUMA resize state machine is functional but produces data only for the NUMA transport.
...
Key directions include: (1)~validating on real CXL hardware (Samsung CMM-D, Intel CXL DIMMs) to assess PCIe bandwidth and 200--400\,ns CXL latency effects; (2)~extending to CXL~3.0 multi-node pooling via a fabric switch to remove the single-machine limitation; (3)~comparing against CXL-native KV stores (CHash~\cite{chash}, MIND~\cite{mind2021}, AIFM~\cite{aifm2020}); (4)~fixing broken RACE/MICA/Cluster builds for direct ratio verification; and (5)~optimizing write-heavy workloads (YCSB-A/F) with per-lane write buffers and CLWB instructions on CXL.
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Literature Review}\label{app:litreview}
% ══════════════════════════════════════════════════════════════════════════════

This appendix surveys the systems and techniques directly relevant to this work.
...
\textbf{RACE}~\cite{race} (\emph{USENIX ATC 2021}) is an extendible hashing index for disaggregated memory using only one-sided RDMA verbs. Outback reports 2.43--5.03$\times$ over RACE on YCSB-A/C.

\textbf{MICA}~\cite{mica} (\emph{USENIX NSDI 2014}) is a single-machine KV store with kernel-bypass networking. In Outback's evaluation it is invoked via two-sided RPC (\textit{RPC-MICA}), making it the weakest baseline at 2.47\,MOPS (YCSB-C, 72T).

\textbf{Cluster RPC} is a cluster-hash approach (similar to DrTM+R~\cite{drtmr}) via two-sided RPC, reaching $\sim$5.41\,MOPS at 72 threads.

\textbf{FaSST}~\cite{fasst} (\emph{OSDI 2016}), \textbf{DrTM+R}~\cite{drtmr} (\emph{OSDI 2018}), and \textbf{ROLEX}~\cite{rolex} (\emph{USENIX FAST 2023}) are included in the codebase but not plotted in this report's graphs.

\subsection*{Key Algorithmic Primitives}

\textbf{Ludo Hashing}~\cite{ludo} (\emph{ACM SIGMETRICS 2020}) achieves $3.76 + 1.05\ell$ bits per KV item, near the information-theoretic minimum, with dynamic insertions. Used as Outback's DMPH seed arrays.

\textbf{YCSB}~\cite{ycsb} (\emph{ACM SoCC 2010}) provides workloads A--F covering read-heavy to write-heavy patterns. All experiments use 50\,M KV pairs, 10\,M key working set, Zipfian distribution.

\subsection*{CXL Memory Disaggregation}

\textbf{CXL}~\cite{cxl} is an open PCIe-based interconnect providing cache-coherent load/store remote memory at 150--300\,ns. CXL~3.0 supports multi-node pooling. In this work, CXL.mem is emulated via NUMA inter-socket shared memory (UPI latency $\sim$100--200\,ns).
