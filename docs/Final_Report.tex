...
\textbf{Ludo Hashing}~\cite{ludo} (\emph{ACM SIGMETRICS 2020}) achieves $3.76 + 1.05\ell$ bits per KV item (near the information-theoretic minimum) with dynamic insertions; it forms Outback's DMPH seed arrays. \textbf{YCSB}~\cite{ycsb} (\emph{ACM SoCC 2010}) provides workloads A--F (read-heavy to write-heavy); all experiments use 50\,M KV pairs, 10\,M key working set, Zipfian distribution. \textbf{CXL}~\cite{cxl} is an open PCIe-based interconnect providing cache-coherent load/store remote memory at 150--300\,ns; CXL~3.0 supports multi-node pooling. In this work, CXL.mem is emulated via NUMA inter-socket shared memory (UPI $\sim$100--200\,ns).

% ══════════════════════════════════════════════════════════════════════════════
\section{Environment Setup}
% ══════════════════════════════════════════════════════════════════════════════

Three hardware environments are compared throughout this report: the original paper's two clusters (CloudLab \texttt{r650} with CX-6 100\,Gbps, and \texttt{r320} with CX-3 56\,Gbps), our RDMA reproduction (CloudLab \texttt{r320}, same hardware as the paper's Fig.~10 cluster), and our CXL/NUMA extension (CloudLab \texttt{c6420}, dual-socket NUMA as a CXL proxy). Table~\ref{tab:setup-diff} lists the key differences and their expected effect on results; full hardware listings appear in Appendix~\ref{app:hw}.
...
\toprule
\textbf{Factor} & \textbf{Paper (r650, Fig.~9)} & \textbf{Paper (r320, Fig.~10)} & \textbf{RDMA (ours)} & \textbf{CXL (c6420)} & \textbf{Expected Impact} \\
\midrule
NIC Speed & CX-6, 100\,Gbps IB & CX-3 MX354A, 56\,Gbps FDR IB & CX-3 MX354A, 56\,Gbps FDR IB & N/A (load/store) & $\sim$2$\times$ lower NIC BW vs.\ r650; like-for-like vs.\ r320 \\
CPU & Platinum 8360Y, 2$\times$36C & Xeon E5-2450, 8C & Xeon E5-2450, 8C & 2$\times$Xeon Gold 6142, 16C & RDMA CPU identical to paper's Fig.~10 \\
Max Threads & 144/shard & 64 (8/node) & 32 (2$\times$16) & 32 (1 socket) & Cannot test high-thread regime of Fig.~9 \\
MN Threads & 1--3 (2 shards) & 4 & 1--3 & none (no MN CPU)$^a$ & RDMA MN count matches Fig.~10 \\
Node Count & 6 (2 shards $\times$ 3) & Up to 9 (1\,MN + 8\,CN) & 3 & 1 & Fewer CN; lower aggregate throughput \\
Memory & 256\,GB & 16\,GB DDR3 & 16\,GB DDR3 & 384\,GB DDR4 & RDMA memory identical to Fig.~10; OOM at 80M \\
Interconnect & PCIe / InfiniBand FDR & InfiniBand FDR & InfiniBand FDR & UPI $\sim$150\,ns & CXL $\sim$20$\times$ lower latency \\
\bottomrule
\end{tabular}}
\vspace{2pt}
