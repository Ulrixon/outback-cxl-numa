...
\end{axis}
\end{tikzpicture}
\caption{RDMA coroutine latency--throughput in Paper Fig.~13 style (1MN 2CN, YCSB-C, 4--32 threads). Thread counts are labeled on the C=2 curve. Filled markers = our r320 / CX-3 FDR reproduction; thin green lines = paper Fig.~13a digitized (CX-6, r650, MT=1): solid=coro=1, dashed=coro=2, dash-dot=coro=3. Both exhibit the ``hockey stick'' shape: latency stays flat until near-saturation, then explodes. Key differences: (1) Our baseline latency $\sim$2\,$\mu$s vs.\ paper $\sim$1\,$\mu$s (CX-6 faster NIC). (2) Saturation throughput: ours C=2 peaks at 5.04\,MOPS vs.\ paper $\sim$5.5\,MOPS. (3) Coro=3 saturates earlier and at lower throughput (4.41\,MOPS at 21\,$\mu$s) than C=2 --- excess queue depth hurts under MN CPU saturation.}
\label{fig:rdma-coro}
\end{figure}
